...
for (holes of input) {
  ...
  rowArray.push(rowmax);
  colArray.push(colmax);

  dugHole = rowmax * 8 + colmax;
  dugHoles.push(dugHole);
...
console.log(`The lowest square is ${min} and the highest square is ${max}`);//Its 51 and 832
...
console.log(`The square that wasnt dug was ${missingSquare}`);
...
console.log(`The pattern of the missing square is ${encoded}`)
...
let rowTotal = rowArray.reduce((total, row) => {
    return total + row
}, 0)
let colTotal = colArray.reduce((total, col) => total + col, 0)
