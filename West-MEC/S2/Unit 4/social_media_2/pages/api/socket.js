...
const SocketHandler = (req, res) => {
  if (res.socket.server.io) {
    console.log("Socket's good mate");
  } else {
    console.log("Sockets's getting ready");
    const io = new Server(res.socket.server);
    res.socket.server.io = io;
    console.log("Socket's good mate");

    //first two lines send data to the server side, last two line receives data from the server side
    io.on("connection", (socket) => {
      ...
      socket.on("sendNewMsg", async ({ userId, msgSendToUserId, msg }) => {
        try {
          ...
          if (receiverSocket)
            ...
            const user = await UserModel.(userId);
          }
      ...
      socket.on("disconnetct", () => removeUser(socket.id));
