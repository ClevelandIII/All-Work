...
const createPost = async (req, res) => {
  ...
  if (!text.length)
    return res.status(401).send("Cmon dude, longer than that...");
...
const getPostById = async (req, res) => {
  try {
    ...
    if (!post) return res.status(404).send("We searched all over and, nowhere to be found...");
...
const deletePost = async (req, res) => {
  try {
    ...
    if (post.user.toString() !== userId) {
      if (user.role === "root") {
        await post.remove();
        return res.status(200).send("Termination Complete");
      } else {
        return res.status(401).send("Not Enough Authority!");
      }
    }
...
const likePost = async (req, res) => {
  try {
    ...
    if (isLiked) return res.status(401).send("Bro, You Already Liked This...");
...
const unlikePost = async (req, res) => {
  try {
    ...
    if (likedIndex === -1) return res.status(403).send("Yeah maybe like this first");

    await post.likes.splice(likedIndex, 1);
    await post.save();

    return res.status(200).send("And so the post was unliked");
  } catch (error) {
    console.log(error);
...
const deleteComment = async (req, res) => {
  try {
    ...
    if (!comment) return res.status(403).send("Invalid Comment");
