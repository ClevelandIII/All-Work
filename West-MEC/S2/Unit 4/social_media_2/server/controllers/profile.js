...
const getFollowing = async (req, res) => {
  try {
    ...
    console.log(error);
    return res.status(500).send("Yo chief theres an issue at.. at.. up getfollowing...");
  }
};
...
const followUser = async (req, res) => {
  try {
    ...
    const isFollowing = user.following.find(
      ...
      if (userToFollowId === userId ) return res.status(401).send('Yeah bud you cant do that...')

    if (isFollowing) return res.status(401).send("You alreaty tracked this person, calm down");
...
const unfollowUser = async (req, res) => {
  try {
    ...
    if (userToUnfollowId === userId ) return res.status(401).send('Yeah bud you cant do that...')
    ...
    if (isFollowingIndex === -1)
      return res.status(401).send("Calm down bub, you havent followed them yet");
